import { Toollee } from "./toollee.js";
import { Choosse } from "./Choosse.js";
import { iconContactDelete } from "./icons.js";
...
export class InputForm {
  ...
  constructor(params) {
    ...
    new Toollee({
      ...
      setUser: {
        // tooltip set: if there are none the default settings will be applied
    ...
    // personal data
    this.$sectionPerson.classList.add(
      "input-form__section",
    ...
    // contacts data
    this.$sectionContacts.classList.add(
      "input-form__section",
    ...
    this.$btnAddContact.append(
      this.$btnAddContactIcon,
      this.$btnAddContactText
    );
    ...
    // service-----------------------------------------------------
    this.$sectionService.classList.add(
      "input-form__section",
      "input-form__section-service"
    );
    this.$btnSubmit.classList.add(
  ...
  checkContainContacts() {
    ...
    if (this.$containerContacts.childNodes.length >= 10)
      this.$btnAddContact.classList.add("visually-hidden");
  ...
  validationStart() {
    ...
    if (
      this.inputs.find((item) => item.valid === false) ||
      this.contacts.find((item) => item.valid === false)
    )
  ...
  contactsUpdate() {
    this.contacts.splice(
      this.contacts.findIndex((item) => item.delete),
      1
    );
  }
  ...
  set validationForm(value) {
    ...
    if (value) {
      for (const item of this.menu.listSelect) {
        if (item.name == "validAutoCorrect" || item.name == "validLang")
          item.disable = false;
      }
    } else {
      this.formValid = true;
